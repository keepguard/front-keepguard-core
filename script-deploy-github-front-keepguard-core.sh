#!/bin/bash
# =============================================================================
# 🚀 MINI MANUAL DE USO — script-deploy-github-front-keepguard-core.sh (front-keepguard-core)
# =============================================================================
# Este script automatiza o ciclo completo de desenvolvimento, build e deploy
# do serviço front-keepguard-core.
#
...
#         ghcr.io/keepguard/front-keepguard-core:develop-<sha>
#         ghcr.io/keepguard/front-keepguard-core:develop-latest
#
#   2. Commit, push, build e deploy imediato no Docker Compose local:
#      $ ./script-deploy-github-front-keepguard-core.sh up
...
#      $ ./script-deploy-github-front-keepguard-core.sh merge main
...
#      $ ./script-deploy-github-front-keepguard-core.sh merge main up
#
#   5. Pipeline Completo em 1 comando (Merge 'main' + Docker Local + K8s Produção):
#      $ ./script-deploy-github-front-keepguard-core.sh merge main up prod
#      OU simplesmente:
#      $ ./script-deploy-github-front-keepguard-core.sh full
...
if [ "$BUILD_BRANCH" = "main" ]; then
    docker build --platform linux/amd64 --build-arg BUILD_MODE=docker -t "${PRIMARY_TAG}" -t "${LATEST_TAG}" -t "${BRANCH_TAG}" -t "${BRANCH_LATEST}" .
else
    docker build --platform linux/amd64 --build-arg BUILD_MODE=docker -t "${PRIMARY_TAG}" -t "${LATEST_TAG}" .
fi
